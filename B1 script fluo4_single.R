...
B1 = read.csv("O:/MBdata/Ca_imaging/fluo4 tmem/2019-12-27/tiff export/exp54/stats.csv", header =F)
...
response_frame = cbind(c(11, 101), c(100, 120))
# Rename the start and end columns
colnames(response_frame) = c("Start", "End")
# Rename the rows with the agonists
rownames(response_frame) = c("Agonist 1", "HiK")
...
Agonists_df = data.frame(c(agonist), c(0.2, 0.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[140,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df)-1,i], "Neuron", "Non-neuron"))
  }
  ...
  dir.create("O:\\MBdata\\Ca_imaging\\fluo4 tmem\\2019-12-27\\tiff export\\exp54\\B1plots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("O:\\MBdata\\Ca_imaging\\fluo4 tmem\\2019-12-27\\tiff export\\exp54\\B1plots", paste("cell_",i,".png", sep=""))
...
CaImaging = ggplot(data = FLUO, aes(x = FLUO$t, y = FLUO$X12.68156425)) + geom_vline(xintercept = 30, color = "forest green") + geom_vline(xintercept = 120, color = "forest green") + geom_vline(xintercept = 180, color = "red") + geom_vline(xintercept = 240, color = "red") + geom_vline(xintercept = 315, color = "purple") + geom_point(alpha = 0.4, color = "blue") + coord_cartesian(xlim = c(0,444), ylim = c(0,3)) + labs(x = "Time (s)", y = "Fluo-4 Normalized Fluorescence (a.u.)", title = "In vitro WT DRG Calcium Imaging Responses to Agonist1, Agonist2, 140mM K") + theme(plot.title = element_text(hjust = 0.5)) + theme_light() + geom_hline(yintercept = 1.0, color = "black", linetype = "solid")
