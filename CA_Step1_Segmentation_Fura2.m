...
%I have commented out the opening of images along the way, except for the
%last segmented labeled image. 
...
%Now find the tif file with this number in the title, and open it for
%segmentation.
%This first part identifies the file with the correct last number.
for i = 1:NumFiles
    ImName = ImageDir(i).name;
    islast = regexp(ImName,num2str(numtiffs));
    if (islast)>0
    HiKframe = ImName;
    end
end
...
%statsL = regionprops(L,imT,'Area', 'MeanIntensity');
...
BigBadCells = find([statsL2.Area]>=3500);
...
LabelFig = figure,imshow(label2rgb(L4, 'jet','k','shuffle'))
...
%print(LabelFig,[FileLoc '\' 'ROI.tif']);
